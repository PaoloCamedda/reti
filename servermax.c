...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        do{
            returnStatus=read(simpleChildSocket,&numToReceve,sizeof(numToReceve));
            if(returnStatus>0){
                if(numToReceve<0){break;}
                if(numToReceve>maxNum){
                    maxNum=numToReceve;
                    ...
                    fprintf(stderr, "Connect Error 404 To client!\n");
